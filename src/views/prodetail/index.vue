...
<script>
import { getComments, getDetail } from '@/api/searchlist'
import defaultImage from '@/assets/default-avatar.png'
import CountBox from '@/components/CountBox.vue'
import { addCart } from '@/api/cart'
// 此处编写的就是 Vue 组件实例的 配置项，通过一定语法，可以直接混入到组件内
...
export default {
  ...
  methods: {
    ...
    async addCart () {
      if (this.loginConfirm()) return
      const { data } = await addCart(this.detail.goods_id, this.totalCount, this.detail.skuList[0].goods_sku_id)
      this.cartTotal = data.cartTotal
      this.showPannel = false
    }
  },
  async created () {
    const { data: { detail } } = await getDetail(this.$route.params.id)
    // console.log(detail)
    this.images = detail.goods_images
    this.detail = detail

    const { data } = await getComments(this.$route.params.id, this.limit)
    // console.log(data)
    this.comments = data
  }
}
