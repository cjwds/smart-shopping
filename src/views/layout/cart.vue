    ...
    <div v-if="isLogin && cartList.length > 0">
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          ...
          <div v-if="isEdit" class="goPay" :class="{ disabled: selCartTotal === 0}" @click="goPay">结算({{ selCartTotal }})</div>
          <div @click="del" v-else class="delete" :class="{ disabled: selCartTotal === 0}">删除({{ selCartTotal }})</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'selCartTotal', 'selPrice',
      'isAllChecked', 'selCartList'
    ])
  },
  name: 'CartPage',
  created () {
    ...
    goPay () {
      if (this.selCartTotal > 0) {
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCartList.map(ele => ele.id).join(',')
          }
        })
      }
    },
